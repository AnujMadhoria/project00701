...
const getAllProblem = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, data, "event data fetched successfully"));
});
...
const addProblem = asyncHandler(async (req, res) => {
  ...
  // Process tags: assume it can be a comma-separated string or an array
  let tagsArray = [];
  if (tags) {
  ...
  return res
    .status(200)
    .json(
      new ApiResponse(200, newProblem, "Problem post created successfully")
    );
});
...
const deleteCommentOnProblem = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, response, "successfully deleted"));
});
